...
class DecoderCNN(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        for i in range(len(channels) - 1):
            modules.append(nn.ConvTranspose2d(in_channels=channels[i], out_channels=channels[i + 1], **conv_params))
            if i < len(channels) - 2:
...
class VAE(nn.Module):
    def __init__(self, features_encoder, features_decoder, in_size, z_dim):
        ...
        # ====== YOUR CODE: ======
        self.mu_alpha = nn.Linear(n_features, z_dim, bias=True)
        self.log_variance = nn.Linear(n_features, z_dim, bias=True)
        self.latent_transformer = nn.Linear(z_dim, n_features, bias=True)
        # ========================
    ...
    def encode(self, x):
        ...
        h = h.view(h.size(0), -1)
        mu = self.mu_alpha(h)
        log_sigma2 = self.log_variance(h)
    ...
    def decode(self, z):
        ...
        # ====== YOUR CODE: ======
        x_rec = self.features_decoder(self.latent_transformer(z).reshape(-1, *self.features_shape))
        # ========================

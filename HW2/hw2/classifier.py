...
class BinaryClassifier(Classifier):
    ...
    def _classify(self, y_proba: Tensor):
        ...
        #  Output should be a (N,) integer tensor.
        # ====== YOUR CODE: ======
        return torch.where(y_proba[:,self.positive_class] >= self.threshold, 1, 0).type(torch.int32)
        # ========================
...
def select_roc_thresh(
    ...
    dist = torch.norm(torch.stack([torch.tensor(fpr),torch.tensor(tpr)],dim=1)-torch.tensor([0, 1]),dim=1)
    optimal_thresh_idx = torch.argmin(dist)

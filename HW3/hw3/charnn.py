...
def onehot_to_chars(embedded_text: Tensor, idx_to_char: dict) -> str:
    ...
    # ====== YOUR CODE: ======
    result = "".join([idx_to_char[i.argmax().item()] for i in embedded_text])
    # ========================
    return result
...
def chars_to_labelled_samples(text: str, char_to_idx: dict, seq_len: int, device="cpu"):
    ...
    samples = chars_to_onehot(text[:-1], char_to_idx).reshape((N, seq_len, len(char_to_idx))).to(device)
    labels = torch.tensor([char_to_idx[c] for c in text[1:]]).reshape((N, seq_len)).to(device)
    # ========================
...
def hot_softmax(y, dim=0, temperature=1.0):
    ...
    # ====== YOUR CODE: ======
    result = torch.softmax(y/temperature, dim=dim) if temperature != 0 else torch.softmax(y/1e-100, dim=dim)
    # ========================
    return result
...
def generate_from_model(model, start_sequence, n_chars, char_maps, T):
    ...
    h = None
    with torch.no_grad():
        curr_c = start_sequence
        for _ in range(n_chars - len(start_sequence)):
            y, h = model(chars_to_onehot(curr_c, char_to_idx).unsqueeze(0).type(torch.FloatTensor).to(device),h)
            c_prob = hot_softmax(y[0, -1, :], 0, temperature=T)
            curr_c = idx_to_char[torch.multinomial(c_prob, 1).item()]
            out_text += curr_c
    # ========================
...
class SequenceBatchSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        idx = [j + i * n for i in range(self.batch_size) for j in range(n)]
        # ========================
        return iter(idx)
...
class MultilayerGRU(nn.Module):
    ...
    def __init__(self, in_dim, h_dim, out_dim, n_layers, dropout=0):
        ...
        for layer in range(n_layers):
            ...
            zx = nn.Linear(input_dim, self.h_dim, bias=False)
            zh = nn.Linear(self.h_dim, self.h_dim, bias=True)
            rx = nn.Linear(input_dim, self.h_dim, bias=False)
            rh = nn.Linear(self.h_dim, self.h_dim, bias=True) 
            gx = nn.Linear(input_dim, self.h_dim, bias=False)
            gh = nn.Linear(self.h_dim, self.h_dim, bias=True)  
            drop_out = nn.Dropout(p=dropout)
            
            self.layer_params.append((zx, zh, rx, rh, gx, gh, drop_out))
            
            self.add_module(f"zx_{layer}", zx)
            self.add_module(f"zh_{layer}", zh)
            self.add_module(f"rx_{layer}", rx)
            self.add_module(f"rh_{layer}", rh)
            self.add_module(f"gx_{layer}", gx)
            self.add_module(f"gh_{layer}", gh)
    ...
    def forward(self, input: Tensor, hidden_state: Tensor = None):
        ...
        for c in range(seq_len):
            ...
            for i in range(len(layer_states)):
                h = layer_states[i]
                zx, zh, rx, rh, gx, gh, drop_out = self.layer_params[i]
                if i != 0:
                    x = layer_states[i-1]
                z = sigmoid(zx(x) + zh(h))
                r = sigmoid(rx(x) + rh(h))
